...
router.get("/api/:roomName", (req, res) => {
  ...
  connection.query(
    `SELECT * FROM rooms WHERE roomName = '${req.params.roomName}'`,
    (err, results) => {
      ...
      ORDER BY rooms_links.lastModified ASC`,
        ...
        INNER JOIN users
          ON users.userId = rooms_links.userId
        WHERE rooms_links.played = 1 && rooms_links.roomId = ${
          results[0].roomId
        }
        ...
        const returnObj = {
          status: 404,
          message:
            "Unable to find the room. Please try a new room, or create this one."
        };
        res.send(returnObj);
      }
    }
...
router.post("/api/create/room", (req, res) => {
  connection.query(
    `INSERT INTO rooms (roomName, adminUser, pass) VALUES ('${
      req.body.roomName
    }', '${req.body.adminUserId}', '${req.body.pass}')`,
    err => {
      if (err) {
        console.log(err);
...
router.post("/api/remove", (req, res) => {
  ...
  `,
    err => {
      ...
      res.send({
        status: 200,
        message: `Removed video at ID: ${req.body.linkId} from ${
          req.body.roomId
        }`
      });
    }
...
router.post("/api/played", (req, res) => {
  connection.query(
    `
  UPDATE rooms_links
    SET played = 1, upvotes = ${req.body.upvotes}, downvotes= ${
      req.body.downvotes
    }
  WHERE linkId = ${req.body.linkId} && roomId = ${req.body.roomId}
  `,
...
router.post("/api/addSong", (req, res) => {
  ...
  connection.query(
    `
  INSERT INTO links (linkName, linkUrl, linkThumbnail) VALUES ('${
    req.body.title
  }', '${req.body.video_id}', '${req.body.thumbnail}')`,
    addErr => {
      if (addErr) {
        ...
        WHERE linkUrl='${req.body.video_id}'`,
            (dupErr, dupId) => {
              ...
              connection.query(
                `
          INSERT INTO rooms_links (linkId, roomId, userId, played, upvotes, downvotes) VALUES (${
            dupId[0].linkId
          }, ${req.body.roomId}, ${req.body.userId}, 0, 0, 0)`,
                roomLinkErr => {
        ...
        connection.query(
          `
      INSERT INTO rooms_links (linkId, roomId, userId, played, upvotes, downvotes) VALUES (LAST_INSERT_ID(), ${
        req.body.roomId
      }, ${req.body.userId}, 0, 0, 0)`,
          roomLinkErr => {
